    ...
    <div className="min-h-screen bg-[#F0F3FB]">
      <header className="bg-white shadow-sm">
        <nav className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <h1 className="text-2xl font-serif text-gray-800 font-bold">The Vault</h1>
            <button
      ...
      <main className="container mx-auto p-4 sm:p-6 lg:p-8">
        <div className="flex justify-end mb-6">
            <Link href="/capsules/create" className="inline-flex items-center justify-center rounded-md bg-black px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {capsules.length > 0 ? (
                            ...
                            return (
                                <div key={capsule._id}>
                                     <div className="bg-white rounded-lg shadow p-6 h-full border border-gray-200 opacity-80">
                                        <div className="flex justify-between items-start">
                                            <h3 className="text-xl font-bold text-gray-500 pr-2 truncate">{capsule.title}</h3>
                                            <Lock className="text-gray-400 flex-shrink-0" />
                                        </div>
                                        <p className="text-sm text-gray-500 mt-2">
                                            Unlocks on: {new Date(capsule.unlockDate).toLocaleDateString()}
                                        </p>
                                        <p className="text-sm font-semibold mt-4 inline-block px-3 py-1 rounded-full text-gray-700 bg-gray-100">
                    ...
                    <div className="col-span-full text-center py-20">
                        <h2 className="text-2xl font-semibold text-gray-700">Your Vault is Empty</h2>
                        <p className="mt-2 text-gray-500">Time to send a message to your future self.</p>
                    </div>
                )}
            </div>
        )}
